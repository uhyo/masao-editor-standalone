...
export function loadFileAsGame(file: File): Promise<MasaoJSONFormat | null> {
  return new Promise(resolve => {
          ...
          .then(game => {
            if (game == null) {
              return Promise.reject(
                new Error(`${file.name}から正男を読み込めませんでした。`),
              );
            } else {
              return game;
            }
    ...
    if (file.type === 'text/html' || /\.html?$/i.test(file.name)) {
      resolve(
        loadText(file)
          .then(masao.load.html)
          .then<masao.format.MasaoJSONFormat>(game => {
            if (game == null) {
              return Promise.reject(
                new Error(`${file.name}から正男を読み込めませんでした。`),
              );
...
function loadHTML(data: string): Promise<any> {
  return new Promise<any>((resolve, _reject) => {
    ...
    // canvas正男を探す
    const scripts = Array.from<HTMLScriptElement>(
...
function findCanvasParams(text: string): Record<string, string> | null {
  ...
  outerloop: while (index < len) {
    ...
    for (; index < len; index++) {
      ...
      if (
        (state === 0 || state === 3 || state === 4 || state === 7) &&
