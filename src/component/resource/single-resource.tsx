...
export default class SingleResourceComponent extends React.Component<
  IPropSingleResource,
  IStateSingleResource
> {
  ...
  render() {
    const { size, resource: { filename, blob }, requestDelete } = this.props;
    const { resourceURL } = this.state;

    const imgClass =
      size === 'large' ? styles.singleResourceImgBig : styles.singleResourceImg;

    const media =
      size !== 'small' && resourceURL != null ? (
