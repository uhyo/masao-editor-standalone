...
export default class MediaComponent extends React.Component<IPropMedia, IStateMedia>{
    ...
    render(){
        ...
        const nameInput = cmd == null ? null :
            <input value={cmd.name} onChange={this.handleInput}/>;

        return <div className={styles.mediaWrapper}>
            <div className={styles.mediaItem}>
                <ParamList current={current} contents={params} requestChange={this.handleParamChange}/>
            </div>
            <div className={styles.mediaItem}>
                <ParamList current={mediaKey} contents={recs2} requestChange={this.handleResourceChange}/>
            </div>
            <div className={styles.mediaItem}>
                <p><b>{current}</b></p>
                {param.data[current] != null ? <p>{param.data[current].description}</p> : null}
                {nameInput}
            </div>
        </div>;
    }
